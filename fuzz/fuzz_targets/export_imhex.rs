#![no_main]
//! Fuzz target: feed an arbitrary Format Hypothesis IR to the ImHex exporter
//! (FR-37, NFR-2).
//!
...
//! exports it to an ImHex pattern under libFuzzer's coverage-guided search. Any
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(output) = export(&format, ExportFormat::ImHex) {
        let again = export(&format, ExportFormat::ImHex).expect("re-export is infallible");
        assert_eq!(output, again, "ImHex export is not deterministic");
    }
});
